...
def main():
    ...
    # Analisa os rótulos e cria um mapeamento entre cada rótulo único e um valor numérico exclusivo
    labelEncoder.fit(trainingLabels)
    # Transforma os rótulos originaisem valores numéricos
    trainingLabels = labelEncoder.transform(trainingLabels)
    ...
    embeddingDim = 16
    vocabularySize = 1000
    # Token para palavras não encontradas ou fora do vocabulário(vocabularySize)
    oovToken = "<OOV>"
    maxLenght = 20                  # Comprimendo máximo dos tokens

    tokenizer = Tokenizer(num_words=vocabularySize,
                          oov_token=oovToken)  # Intancia o tokenizador
    # Alimentando tokenizador com as mensagens de treinamento
    ...
    # Deixa todos os tokens no mesmo tamanho
    paddedSequences = pad_sequences(
        sequences, maxlen=maxLenght, truncating='post')
    ...
    history = model.fit(paddedSequences, np.array(
        trainingLabels), epochs=epocs)
    ...
    with open('./src/label_encoder.pickle', 'wb') as ecn_file:
